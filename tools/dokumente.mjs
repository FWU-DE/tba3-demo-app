...
export const githubUrl = (dok) =>
  `https://github.com/${QUELLE.repo}/blob/${QUELLE.zweig}/${QUELLE.ordner}/${dok.datei}`;
...
${inhalt}
  ...
  <footer class="dokument-fuss">
    <a href="${githubUrl(dok)}">${zweisprachig({ de: 'Original auf GitHub', en: 'Original on GitHub' })}</a>
...
export function uebersicht(dokumente, standDaten = null) {
  ...
  <header class="uebersicht-kopf">
    ...
    <p>${zweisprachig({
      de: `Die erklärenden Texte zur Auswertungsschnittstelle — hier zu lesen, nicht erst auf GitHub.
           Sie stammen aus <a href="https://github.com/${QUELLE.repo}">${QUELLE.repo}</a> und liegen in
           dieser Seite als Kopie${datum ? ` (Stand ${datum.de})` : ''}. Die Endpunkte selbst stehen in der
           <a href="/schnittstelle">API-Referenz</a>.`,
      en: `The explanatory texts on the reporting API — readable here, not only on GitHub.
           They come from <a href="https://github.com/${QUELLE.repo}">${QUELLE.repo}</a> and are kept in this
           site as a copy${datum ? ` (as of ${datum.en})` : ''}. The endpoints themselves are in the
  ...
  return rahmen({
    ...
    beschreibung: {
      de: 'Konzepte, Endpunkt-Referenz und Rezepte zur TBA3-Auswertungsschnittstelle.',
      en: 'Concepts, endpoint reference and recipes for the TBA3 reporting API.',
    },
    koerper,
